...
const cart = () => {
  ...
  onMounted(() => {
    axiosAdmin
      ...
      .then((response) => {
        // Assuming currency is needed, define it first
        const currency = ref(response.data.currency);
        frontWarehouse.value = response.data.warehouse;
      });
  });
  ...
  const getRecursiveCategories = (response, excludeId = null) => {
    ...
    response.data.forEach((responseArray) => {
      if (
        excludeId === null ||
    ...
    listArray.forEach((node) => {
      ...
      const parentIndex = listArray.findIndex((el) => el.id === node.parent_id);
